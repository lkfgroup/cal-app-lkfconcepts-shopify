...
export default function Order({ authAxios, shop }) {
  ...
  const getOrder = async (id) => {
    try {
      let response = await authAxios.get(`/api/draft_orders/${id}`);
  ...
  const fetch = useCallback(async (keySearch, after, before, limit) => {
    ...
    const list = await authAxios.get(
      `/api/draft_orders?keySearch=${keySearch || ''}&after=${after || ''}&before=${before || ''}&limit=${limit || 40}`
    );
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    await authAxios.post('/api/fulfill_draft_order', {
      ...formik.values,
      shop,
      draft_order_id: parseId(orderData.id),
