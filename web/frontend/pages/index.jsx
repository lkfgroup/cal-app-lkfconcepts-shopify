...
export default function Index({ shop, authAxios }) {
  ...
  const fetch = useCallback(async (keySearch, after, before, limit) => {
    ...
    const list = await authAxios.get(
      `/api/products?keySearch=${keySearch || ''}&after=${after || ''}&before=${before || ''}&limit=${limit || 100}`
    );
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    await authAxios.post('/api/edit-settings', { ...formik.values, product_id: productId, shop });
      ...
      <Layout>
        <Layout.Section>
          ...
          >
            ...
            {/* <Modal.Section>
             <LegacyCard title="Delivery Slots">
